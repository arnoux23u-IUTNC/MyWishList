...
class ListView
{
    ...
    private function showList()
    {
        ...
        $dataHeader = "\n\t".match(filter_var($this->request->getQueryParam('state'), FILTER_SANITIZE_STRING) ?? ""){
            "update" => "<div class='popup'>{$lang['list_updated']}</div>",
            "newItem"  => "<div class='popup'>{$lang['list_item_added']}</div>",
            "modItem"  => "<div class='popup'>{$lang['list_item_updated']}</div>",
            "delItem"  => "<div class='popup warning'>{$lang['list_item_deleted']}</div>",
            default => ""
        };    
        $warnEdit = "\n\t".match(filter_var($this->request->getQueryParam('info'), FILTER_SANITIZE_STRING) ?? ""){
            "ok" => "<div class='popup'>{$lang['image_saved']}</div>",
            "typeerr"  => "<div class='popup warning'>{$lang['image_type_error']}</div>",
            "sizeerr"  => "<div class='popup warning'>{$lang['image_size_error']}</div>",
            "writeerr"  => "<div class='popup warning'>{$lang['image_write_error']}</div>",
            "fileexist"  => "<div class='popup warning'>{$lang['image_exists']}</div>",
            "error"  => "<div class='popup warning'>{$lang['image_error']}</div>",
            default => ""
        };        
        $html = genererHeader("{$lang['list']} $l->no - MyWishList", ["list.css"]) . <<<EOD
            <h2>$l->titre</h2>$dataHeader$warnEdit
            <p>{$lang['list_associated_user']} : $l->user_id</p>
            <p>{$lang['description']} : $l->description</p>
            <p>{$lang['expiration_date']} : $l->expiration</p>
            <a href="$routeAddItem">{$lang['list_add_item']}</a>
            <div class='items_list'>
                <div class='box'>
                    <h2>{$lang['list_associated_items']} $l->no</h2>
                    <ul>
        EOD;
        foreach ($l->items as $pos => $item) {
            ...
            $item_res = ($this->request->getCookieParam("typeUser") === 'participant') ? (!empty($reserved) ? "<p>{$lang['list_reserved_by']} $reserved->user_id -> $reserved->message</p>" : ($l->isExpired() ? "<p><i>{$lang['reservation_not_possible']}</i></p>" : "\n\t\t\t\t\t\t<form method='post' action='#'>\n\t\t\t\t\t\t\t<button class='sendBtn' type='submit' name='sendBtn' title='{$lang['reserve']}'><img src='/assets/img/checkmark.png'/></button>\n\t\t\t\t\t\t</form>\n\t\t\t\t\t")) : (!empty($reserved) ? ($l->isExpired() ? "<p>{$lang['list_reserved_by']} $reserved->user_id -> $reserved->message</p>" : "<p>{$lang['item_reserved']}</p>") : "<p>{$lang['item_unreserved']}</p>");
        ...
        return $html . "\n\t\t\t</ul>\n\t\t</div>\n\t</div>\n\t<a href='$routeListEdit'>{$lang['list_edition']}</a>\n</body>\n</html>";
    }

    private function createList(){
        $routeCreate = $this->container->router->pathFor('lists_create');
        return genererHeader("{$lang['phtml_lists_create']}", ["list.css"]). <<<EOD
            <h2>"{$lang['phtml_lists_create']}"</h2>
            <div>
                <form class='form_container' method="post" action="$routeCreate">
                    ...
                    <label for="exp">{$lang['expiration_date']}</label>
                    <input type="date" name="exp" id="expiration"/>
                    <label for="public_key">{$lang['public_token']}</label>
                    <input type="text" name="public_key" id="public_key" />
                    <button type="submit" name="sendBtn">{$lang['create']}</button>
                </form>
            <div>
        </body>
        </html>
    ...
    private function addItem(){
        ...
        return genererHeader("Items | {$lang['list']} $l->no", ["list.css"]). <<<EOD
            <h2>{$lang['list_add_item']} $l->no</h2>
            <div>
    ...
    private function requestAuth(){
        ...
        return genererHeader("{$lang['list_editing']} - {$lang['auth']}", ["list.css"]). <<<EOD
            <h2>{$lang['list_editing']}</h2>
            <div>
    ...
    private function edit(){
        ...
        return genererHeader("{$lang['list']} $l->no | {$lang["editing"]}", ["list.css"]). <<<EOD
            <h2>{$lang["liste_editing"]} $l->no</h2>
            <div>
                <form class='form_container' method="post" action="$routeListEdit">
                    ...
                    <label for="user">{$lang['list_associated_user']}</label>
